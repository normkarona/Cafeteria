...
async def status_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Keep one clean customer receipt: edit its status instead of sending
    # a new Telegram message for every status change.
    try:
        ...
        await query.answer(
            "Status updated, but customer receipt update failed.",
            show_alert=True
        )
        return
